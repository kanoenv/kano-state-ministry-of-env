...
const ContentManagement = () => {
  ...
  const contentTypes = {
    news: { label: "News & Updates", type: "news" },
    resources: { label: "Resources", type: "resource" },
  ...
  const handleAddContent = async () => {
    try {
      ...
      const contentData = {
        ...
        type: contentTypes[newContent.type as keyof typeof contentTypes].type,
              ...
              {isLoading ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse">
                    ...
                    <tbody>
                      {contentItems.map((item) => {
                        const eventDetails = JSON.parse(item.content || '{}');
                        return (
                          <tr key={item.id} className="border-b border-gray-200 hover:bg-gray-50">
                            <td className="px-4 py-4 text-sm text-gray-900">{item.title}</td>
